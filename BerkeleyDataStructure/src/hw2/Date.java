...
class Date {
	...
	public Date(String s) {
		int prevIndex = 0;
		int curIndex = 0;
		curIndex = s.indexOf('/', prevIndex);
		int month = Integer.parseInt(s.substring(prevIndex, curIndex));
		prevIndex = curIndex + 1;
		curIndex = s.indexOf('/', prevIndex);
		int day = Integer.parseInt(s.substring(prevIndex, curIndex));
		int year = Integer.parseInt(s.substring(curIndex + 1));
	...
	public int difference(Date d) {
		...
		int difference = 0;
		for (int curYear = smaller.year; curYear < larger.year; ++curYear) {
			difference += (isLeapYear(curYear) ? 366 : 365);
		}
		difference = difference + larger.dayInYear() - smaller.dayInYear();
		return isBefore ? -difference : difference;
	}
