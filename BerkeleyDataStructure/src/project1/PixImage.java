...
public class PixImage {
	...
	public String toString() {
		...
		printHelper(blue);
		return "PixImage [height=" + height + ", width=" + width;
	}

	private void printHelper(short[][] color) {
	...
	public PixImage boxBlur(int numIterations) {
		if (numIterations <= 0) {
			...
			for (int i = 0; i < numIterations; ++i) {
				newImage = boxBlurOneTime(prevImage);
				prevImage = newImage;
			}
			return newImage;
		}
	...
	private PixImage boxBlurOneTime(PixImage prevImage) {
		PixImage newImage = new PixImage(width, height);
		for (int i = 0; i < width; ++i) {
			for (int j = 0; j < height; ++j) {
				newImage.red[i][j] = avgNeighbor(prevImage.red, i, j);
				newImage.green[i][j] = avgNeighbor(prevImage.green, i, j);
				newImage.blue[i][j] = avgNeighbor(prevImage.blue, i, j);
			}
		}
		return newImage;
	}

	private short avgNeighbor(short[][] color, int x, int y) {
		int sum = 0;
		short count = 0;
		...
		return (short) (count != 0 ? sum / count : 0);
	}
	...
	public PixImage sobelEdges() {
		...
		for (int i = 0; i < width; ++i) {
			for (int j = 0; j < height; ++j) {
				int gxr = calGx(this.red, i, j);
				int gyr = calGy(this.red, i, j);
				int gxg = calGx(this.green, i, j);
				int gyg = calGy(this.green, i, j);
				int gxb = calGx(this.blue, i, j);
				int gyb = calGy(this.blue, i, j);
				int energy = gxr * gxr + gyr * gyr + gxg * gxg + gyg * gyg
						+ gxb * gxb + gyb * gyb;
				short intensity = mag2gray(energy);
				newImage.red[i][j] = newImage.green[i][j] = newImage.blue[i][j] = intensity;
			}
		}
		return newImage;
	}

	private int calGx(short[][] color, int x, int y) {
		return getValue(color, x - 1, y - 1) - getValue(color, x + 1, y - 1)
				+ 2 * getValue(color, x - 1, y) - 2 * getValue(color, x + 1, y)
				+ getValue(color, x - 1, y + 1) - getValue(color, x + 1, y + 1);
	}

	private int calGy(short[][] color, int x, int y) {
		return getValue(color, x - 1, y - 1) + 2 * getValue(color, x, y - 1)
				+ getValue(color, x + 1, y - 1) - getValue(color, x - 1, y + 1)
				- 2 * getValue(color, x, y + 1) - getValue(color, x + 1, y + 1);
	}

	private short getValue(short[][] color, int x, int y) {
		if (x < 0) {
			x = 0;
		}
		if (x >= width) {
